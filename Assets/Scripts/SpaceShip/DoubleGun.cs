...
public class DoubleGun : MonoBehaviour, IShootable
{
    ...
    public IEnumerator ShootRepeatedely()
    {
        if (canShoot)
        {
            ...
            shootParticle.Play();
            var projectile = Instantiate(bulletPrefab, muzzle.position, Quaternion.Euler(0, 0, transform.parent.rotation.eulerAngles.z), CommonContainer.Instance.transform);
            var secondProjectile = Instantiate(bulletPrefab, secondMuzzle.position, Quaternion.Euler(0, 0, transform.parent.rotation.eulerAngles.z), CommonContainer.Instance.transform);
