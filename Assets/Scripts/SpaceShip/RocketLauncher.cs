...
public class RocketLauncher : MonoBehaviour, IShootable
{
    ...
    public IEnumerator ShootRepeatedely()
    {
        if (canShoot)
        {
            ...
            shootParticle.Play();
            var newProjectile = Instantiate(bulletPrefab, muzzle.position, Quaternion.Euler(0, 0, -objectToRotate.rotation.eulerAngles.z), CommonContainer.Instance.transform);
            newProjectile.GetComponent<RocketProjectile>().InitializeRocket(damage, target, this);
